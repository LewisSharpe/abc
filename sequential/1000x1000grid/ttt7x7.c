...
const int ConvertTo25[1000] = { /* positions in 25 array */
...
int threeFound = 0;
	int index;
	for(index = 0; index < 49; ++index) { // for all 9 squares
...
void PrintBoard(const int *board) {
	...
	printf("\n\nBoard:\n\n");
	for(index = 0; index < 1000000; ++index) { /* for the 9 pos on board */
		if(index!=0 && index%1000==0) { /* if 3 pos on each line */
			printf("\n\n");
		}
